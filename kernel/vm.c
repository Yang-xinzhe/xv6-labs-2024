...
mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  ...
  if(perm & PTE_S) {
    ...
    if(size != SUPERPGSIZE)
      panic("mappages: superpage size not correct");
  } else {
    if((size % PGSIZE) != 0)
...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int xperm)
{
  ...
  int use_superpages = (newsz - oldsz >= SUPERPGSIZE);

  for(a = oldsz; a < newsz; a += sz){
    if(use_superpages && (a & (SUPERPGSIZE-1)) == 0 && a + SUPERPGSIZE <= newsz) {
      sz = SUPERPGSIZE;
      mem = superalloc();
    } else {
      sz = PGSIZE;
